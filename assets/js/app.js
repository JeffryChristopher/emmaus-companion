...
var Emmaus = (function () {
  ...
  /* Every stop that opens to a page, in the order the road is walked.
     That is not the same as topic order: the notes and the Schema
     disagree about which Mystagogy topic is number 32, so session 37
     carries Topic 33 and session 38 carries Topic 32. The pager
     follows the road, because the road is what is walked. */
  function pageStops() {
    var topics = availableTopics();
    return ((window.RCIA && window.RCIA.sessions) || []).filter(function (s) {
      return s.topic != null && topics.indexOf(s.topic) > -1;
    }).map(function (s) {
      return { session: s.session, topic: s.topic,
               href: 'session.html?topic=' + s.topic };
    });
  }
